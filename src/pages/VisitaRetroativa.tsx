...
const VisitaRetroativa = () => {
  ...
  const handleSubmitFinal = async (payload: {
    ...
    toast({
      ...
      description: result.success ? "Sua visita manual retroativa foi salva e enviada para aprovação do Analista!" : result.message,
    ...
    <div className="max-w-3xl mx-auto space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-foreground">Visita Retroativa</h1>
            <p className="text-sm font-semibold text-muted-foreground mt-1">
              Registre uma visita retroativa manualmente. Estará sujeita à aprovação do Analista.
            </p>
          </div>
        </div>
              ...
              <CardContent className="p-4 sm:p-6 pt-5 sm:pt-6">
                <RadioGroup value={form.tipo_visita} onValueChange={(v) => handleChange("tipo_visita", v)} className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {tipoVisitaOptions.map((option) => {
                    const isDisabled = !["FDS", "COACHING ROTA BASICA COM VENDEDOR", "FOCO RGB", "FOCO MAIORES QUEDAS RGB", "MAIORES POTENCIAS BASE DE COMPRAS CGB", "MAIORES POTENCIAS BASE DE COMPRAS RGB", "MAIORES POTENCIAS COMPASS em RGB BAR"].includes(option);
                    return (
        ...
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-500">
          {["FDS", "FOCO RGB", "FOCO MAIORES QUEDAS RGB", "MAIORES POTENCIAS BASE DE COMPRAS RGB", "MAIORES POTENCIAS COMPASS em RGB BAR"].includes(form.tipo_visita) && (
...
export default VisitaRetroativa;
